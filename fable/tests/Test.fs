...
let run () = async {
  ...
  model.compile (ModelCompileArgs.props [ Loss MeanSquaredError; Optimizer StocasticGradientDecent ])

  let xs = Tf.tensor2d ([| -1.0; 0.0; 1.0; 2.0; 3.0; 4.0 |], [| 6; 1 |])
  let ys = Tf.tensor2d ([| -3.0; -1.0; 1.0; 3.0; 5.0; 7.0 |], [| 6; 1 |])

  printfn "Fitting model"
...
let coreTests =
    testList "Core tests" [
        ...
        testAsync "Test - 1" {
            let! value = run ()
            value.predict(Tf.tensor2d ([| 20.0 |], [| 1; 1 |])).print ()
